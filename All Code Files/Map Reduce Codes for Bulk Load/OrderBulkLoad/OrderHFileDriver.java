...
public class OrderHFileDriver {
    public static void main(String[] args) throws Exception {
        if (args.length != 3) {
            System.err.println("Usage: OrderHFileDriver <input path> <output path> <table name>");
            System.exit(-1);
        }
        ...
        Job job = Job.getInstance(conf, "Order HFile Generator");
        job.setJarByClass(OrderHFileDriver.class);

        // Configure job inputs and outputs
        job.setMapperClass(OrderHFileMapper.class);
        job.setReducerClass(OrderHFileReducer.class);
