...
public class CustomerHFileDriver {
    public static void main(String[] args) throws Exception {
        if (args.length != 3) {
            System.err.println("Usage: CustomerHFileDriver <input path> <output path> <table name>");
            System.exit(-1);
        }
        ...
        Job job = Job.getInstance(conf, "Customer HFile Generator");
        job.setJarByClass(CustomerHFileDriver.class);

        // Configure job inputs and outputs
        job.setMapperClass(CustomerHFileMapper.class);
        job.setReducerClass(CustomerHFileReducer.class);
